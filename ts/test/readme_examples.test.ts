...
//     (\`new DatamuseSDK(...)\` / \`DatamuseSDK.test(...)\` become
//     \`DatamuseSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"pet":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new DatamuseSDK(...)\` and any
// \`DatamuseSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+DatamuseSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/DatamuseSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
