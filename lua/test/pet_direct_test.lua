...
describe("PetDirect", function()
  it("should direct-list-pet", function()
    local setup = pet_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-pet", setup.live and "live" or "unit")
  ...
  it("should direct-load-pet", function()
    local setup = pet_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-pet", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function pet_direct_setup(mockres)
