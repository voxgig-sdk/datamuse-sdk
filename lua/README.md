...
It exposes the API as capitalised, semantic **Entities** — e.g. `client:Pet()` — each with the same small set of operations (`list`, `load`, `create`, `remove`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```
lua/
├── datamuse_sdk.lua    -- Main SDK module
...
The main module (`datamuse_sdk`) exports the SDK constructor
