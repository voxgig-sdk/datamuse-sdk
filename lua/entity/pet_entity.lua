...
local PetEntity = {}
PetEntity.__index = PetEntity


function PetEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PetEntity)
  self._name = "pet"
  self._client = client
...
function PetEntity:get_name()
  return self._name
end


function PetEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PetEntity.new(self._client, opts)
end
...
function PetEntity:mark_deleted()
  self._deleted = true
end


function PetEntity:deleted()
  return true == self._deleted
end


function PetEntity:data_set(args)
  if args ~= nil then
...
function PetEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PetEntity:match_set(args)
  if args ~= nil then
...
function PetEntity:stream(action, args, callopts)
...
function PetEntity:_run_op(ctx, post_done)
  local utility = self._utility
