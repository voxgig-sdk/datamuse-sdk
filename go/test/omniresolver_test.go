...
import (
	...
	omni "github.com/voxgig-sdk/datamuse-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.DatamuseError); is {
				out := map[string]any{
					"name":    "DatamuseError",
...
func omniCtx(arg any, client *sdk.DatamuseSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.DatamuseSDK); is {
			client = live
