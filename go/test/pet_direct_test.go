...
func TestPetDirect(t *testing.T) {
	t.Run("direct-list-pet", func(t *testing.T) {
		setup := petDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-pet", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-pet", func(t *testing.T) {
		setup := petDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-pet", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type petDirectSetupResult struct {
	client *sdk.DatamuseSDK
	calls  *[]map[string]any
	live   bool
	idmap  map[string]any
}

func petDirectSetup(mockres any) *petDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["DATAMUSE_TEST_PET_ENTID"]; ok {
		...
		return &petDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &petDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
