...
class PetEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new PetEntity($this->_client, $opts);
    }

    /**
     * @param Pet|array $args Pet data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = DatamuseHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Pet|array The current Pet data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Pet fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = DatamuseHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Pet fields).
     */
     ...
     * Load a single Pet.
     *
     * @param PetLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed PetLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Pet|array The loaded Pet as an assoc-array at the
     *   SDK boundary; throws DatamuseError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = DatamuseHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *
     * @param PetListMatch|array|null $reqmatch Match filter (any subset
     *   of Pet fields) as an assoc-array; PetListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Pet[]|array A list of Pet items as assoc-arrays at
     *   the SDK boundary; throws DatamuseError on failure (item-5 convention).
     */
     ...
     * Create a new Pet.
     *
     * @param PetCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed PetCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Pet|array The created Pet as an assoc-array at the
     *   SDK boundary; throws DatamuseError on failure (item-5 convention).
     */
     ...
     *
     * @param PetRemoveMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; PetRemoveMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Pet|array The removed Pet as an assoc-array at the
     *   SDK boundary; throws DatamuseError on failure (item-5 convention).
     */
