...
The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->Pet()` — with named operations (`list`/`load`/`create`/`remove`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
$pet = $client->Pet()->list();
print_r(array_map(fn($item) => $item->data_get(), $pet));
```
...
```php
require_once 'datamuse_sdk.php';
$client = new DatamuseSDK($options);
```
...
```php
$client = DatamuseSDK::test($testopts, $sdkopts);
```
...
```php
// list() returns an array of Pet records (throws on error).
$pets = $client->Pet()->list();
```
...
```
php/
├── datamuse_sdk.php          -- Main SDK class
...
The main class (`datamuse_sdk.php`) exports the SDK class
...
// $pet->data_get() now returns the pet data from the last list
