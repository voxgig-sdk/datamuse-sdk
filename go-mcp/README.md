...
MCP server exposing the Datamuse SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `datamuse_list` | `entity`, optional `query` map | First page of records as JSON |
| `datamuse_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `datamuse_list` and
`datamuse_load` tools become available in new sessions.
