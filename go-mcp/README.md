...
[MCP](https://modelcontextprotocol.io) server exposing the Datamuse SDK as
two agent tools — `datamuse_list` and `datamuse_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/datamuse-mcp)
make build

# 2. Provide credentials via the environment
export DATAMUSE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user datamuse \
  -- /absolute/path/to/datamuse-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./datamuse-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// datamuse_list: first page of records
{ "entity": "pet" }
{ "entity": "pet", "query": { } }

// datamuse_load: one record by id
{ "entity": "pet", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/datamuse-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user datamuse \
     -- "$PWD"/dist/*/datamuse-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `datamuse_list` and `datamuse_load` tools now appear
   in new sessions. Ask the agent to *"list pet using datamuse"*
   and it calls `datamuse_list` with `{"entity":"pet"}`.
...
```sh
export DATAMUSE_APIKEY=sk_live_xxx            # API key
export DATAMUSE_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `datamuse_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `datamuse_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `DATAMUSE_APIKEY` | API key sent with every request. |
| `DATAMUSE_BASE` | Optional override of the API base URL. |
...
```sh
./datamuse-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"datamuse_load","arguments":{"entity":"pet","query":{"id":1}}}}'
```
