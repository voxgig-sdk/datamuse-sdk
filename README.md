...
Learn more about Datamuse API at [datamuse.com](http://datamuse.com).
...
This SDK exposes the API as a small set of **semantic entities** — Pet — that you
...
const items = await client.Pet().list()
```
...
const client = DatamuseSDK.test({
  entity: {
    pet: {
      test01: { id: 'test01', pet: {}, name: 'example_name' },
    },
  },
})
...
```go
client := sdk.Test()
result, err := client.Pet(nil).List(
    nil, nil,
)
```
...
$pets = $client->Pet()->list();
print_r(array_map(fn($item) => $item->data_get(), $pets));
