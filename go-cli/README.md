...
boru-driven command-line client **and** interactive REPL for the Datamuse
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/datamuse-cli)
make build
...
./datamuse-cli list pet
./datamuse-cli load 1 pet            # {id:1} shorthand
./datamuse-cli load '{id:1}' pet       # explicit match map

# 5. Override the API base URL for a single call
DATAMUSE_BASE=https://api.example.com ./datamuse-cli list pet
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/datamuse-cli
   ```
...
```sh
./datamuse-cli load 1 pet          # scalar shorthand for {id:1}
./datamuse-cli load '{id:1}' pet     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `datamuse>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `pet`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/datamuse-cli`. |
...
(`./datamuse-cli <expr>`) and interactively (the REPL), and expressions compose the
