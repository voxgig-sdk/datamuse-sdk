...
class FtCtrl extends DatamuseControl
{
...
class FtClient
{
    ...
    private DatamuseUtility $utility;

    public function __construct(DatamuseUtility $utility, array $options)
    {
    ...
    public function get_utility(): DatamuseUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (DatamuseContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public DatamuseContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (DatamuseContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = DatamuseConfig::make_feature($name);
    ...
    private static function buildUrl(DatamuseSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof DatamuseError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new DatamuseResponse($fetched);
        }
    ...
    private function fail(DatamuseContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (DatamuseContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
