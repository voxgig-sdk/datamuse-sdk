...
// Datamuse SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class DatamuseMakePoint
{
    public static function call(DatamuseContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof DatamuseError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = DatamuseHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = DatamuseHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
