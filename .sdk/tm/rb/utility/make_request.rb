...
module DatamuseUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = DatamuseResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = DatamuseResponse.new(fetched)
    else
