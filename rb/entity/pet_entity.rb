...
class PetEntity
  ...
  def make
    opts = @_entopts.dup
    PetEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = DatamuseHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = DatamuseHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #   as an empty match, so client.Pet.load works with no args).
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = DatamuseHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [PetListMatch, Hash, nil] match filter (any subset of
  #   Pet fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
