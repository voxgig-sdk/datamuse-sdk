...
require_relative 'Datamuse_types'
...
class DatamuseSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = DatamuseHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = DatamuseHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, DatamuseFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = DatamuseHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = DatamuseHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = DatamuseHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => DatamuseError.new(
        "#{op}_allow",
        "DatamuseSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = DatamuseHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = DatamuseError.new(
        "graphql_error", "DatamuseSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
