...
class DatamuseTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            DatamuseTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = DatamuseTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = DatamuseTestRunner.getenv("DATAMUSE_TEST_LIVE")
        override = DatamuseTestRunner.getenv("DATAMUSE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = DatamuseTestRunner.getenv(key)
        ...
        explain = DatamuseTestRunner.getenv("DATAMUSE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["DATAMUSE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if DatamuseTestRunner._test_control is not None:
            return DatamuseTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                DatamuseTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            DatamuseTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = DatamuseTestRunner.load_test_control()
...
def load_env_local():
    DatamuseTestRunner.load_env_local()


def env_override(m):
    return DatamuseTestRunner.env_override(m)


def entity_data(v):
    return DatamuseTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return DatamuseTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return DatamuseTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return DatamuseTestRunner.load_test_control()


def live_client_options():
    return DatamuseTestRunner.live_client_options()


def live_delay_ms():
    return DatamuseTestRunner.live_delay_ms()
